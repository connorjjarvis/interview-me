...
- **Flag:** An `IDisposable` (`HttpClient` misuse aside, think `FileStream`, `SqlConnection`, or a `CancellationTokenSource` that uses `CancelAfter`/`.Token.WaitHandle`) constructed without a `using`/`using var`/`try`-`finally` that guarantees disposal.
